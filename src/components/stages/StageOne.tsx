...
const StageOne: React.FC<StageOneProps> = ({
  ...
  const handleReset = () => {
    setPatients(SAMPLE_PATIENTS.map(p => ({
      ...p,
      confidence: 0
    })));
  ...
  return <div className="w-full space-y-4">
      ...
      {!predictionsRun && <div className="flex justify-center my-6">
          <Button size="lg" onClick={runPredictions} className="w-full max-w-md">
            Run Model Predictions
          </Button>
        </div>}

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {patients.map(patient => <PatientCard key={patient.id} patient={patient} onSelect={handlePatientSelect} showResults={showResults} />)}
      </div>

      {predictionsRun && !showResults && patients.filter(p => p.selected).length > 0 && <div className="flex gap-2 justify-end">
      ...
      {showResults && <div className="flex gap-2 justify-end">
          <Button variant="outline" onClick={handleReset}>
            <RefreshCcw className="mr-2" />
            Try Again
          </Button>
